...
//				return;
	...
	public void LoginHandleRegister(ActionEvent event) {
		try { this.MainApp.getMenuController().getLoginDialogStage().close(); }
		catch (NullPointerException e){ }
		UserInfo userInfo = new UserInfo();
		boolean RegisterClicked = showUserRegisterDialog(userInfo);
		if (RegisterClicked) {
			this.MainApp.getProjectOverviewController().refreshProjectList();
			this.MainApp.getMenuController().clickLoginSelection();
...
//			this.registerDialogStage.initOwner(null);
			...
			this.UserRegisterDialogController.setDialogStage(this.registerDialogStage);
